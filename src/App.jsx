...
import {
  ...
  Menu,
  X,
} from "lucide-react"; // 🟢 تم إضافة Menu و X
import "./App.css";
import { content, departments, projects, leaders, stats } from "./data";

export default function App() {
  const [lang, setLang] = useState("en");
  const [isScrolled, setIsScrolled] = useState(false); // 🟢 حالة السكرول
  const [mobileMenuOpen, setMobileMenuOpen] = useState(false); // 🟢 حالة قائمة الموبايل

  const t = content[lang];
  const isRTL = lang === "ar";

  // 🟢 كشف السكرول لتغيير شكل الناف بار
  useEffect(() => {
    const handleScroll = () => {
  ...
  return (
    <div className="min-h-screen selection:bg-[#C8102E] selection:text-white relative">
        ...
        <div className="container mx-auto px-6 flex justify-between items-center">
          ...
          >
            <img
              src="/logo.png"
              alt="Elsewedy Logo"
              className={`object-contain transition-all duration-300 ${isScrolled ? "h-10" : "h-12 brightness-0 invert"}`} // اللوجو أبيض وهو شفاف
              style={{
                filter: isScrolled ? "none" : "brightness(0) invert(1)",
              }} // في الحالة الشفافة نخليه أبيض
            />
            <div
          ...
          >
            {navLinks.map((link, i) => (
              <a
                key={i}
                href={`#${link.id}`}
                onClick={() => setMobileMenuOpen(false)} // يغلق القائمة عند الضغط
      ...
      <main dir={isRTL ? "rtl" : "ltr"}>
          ...
          <div className="container mx-auto">
            <div className="flex flex-col lg:flex-row items-center gap-16">
              ...
              >
                ...
                <div
                  className={`absolute -top-10 ${isRTL ? "-left-10" : "-right-10"} w-40 h-40 bg-gray-100 rounded-full -z-10`}
                ></div>
              </motion.div>
        ...
        {/* 🟢 DEPARTMENTS SECTION (ID: departments) 🟢 */}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
              {departments.map((dept) => (
                <motion.div
                  key={dept.id}
                  onClick={() =>
                    dept.link ? window.open(dept.link, "_blank") : null
                  }
                  whileHover={{ y: -5 }}
        ...
        {/* 🟢 STATS SECTION (WATERMARK LOGO & ANIMATED COUNTER) 🟢 */}
          ...
          <div className="container mx-auto px-6 relative z-10">
              ...
              {stats.map((stat) => (
                ...
                >
                  ...
                  <h3 className="text-5xl md:text-7xl font-black mb-3 tracking-tighter flex items-baseline justify-center gap-1">
